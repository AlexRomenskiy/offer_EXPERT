import { useState, useRef, useEffect, useMemo } from 'react';
import CustomSlider from './CustomSlider';
import clsx from 'clsx';

const nicheData = {
  coaching: { dmToSaleRate: 0.15, tripwireToFlagship: 0.25, suggestedTripwire: 37, suggestedFlagship: 297, source: "LeadResponse 2026", insight: "Instagram Direct: 90% відкривають, 15% купують." },
  fitness: { dmToSaleRate: 0.18, tripwireToFlagship: 0.28, suggestedTripwire: 27, suggestedFlagship: 197, source: "Unkoa Instagram 2025", insight: "Фітнес-ніша: 60% відповідають на DM." },
  education: { dmToSaleRate: 0.12, tripwireToFlagship: 0.22, suggestedTripwire: 27, suggestedFlagship: 247, source: "Creatorflow 2026", insight: "Освітні продукти: 12% конверсія." },
  beauty: { dmToSaleRate: 0.14, tripwireToFlagship: 0.20, suggestedTripwire: 17, suggestedFlagship: 147, source: "Napolify 2025", insight: "76% клієнтів знаходять б'юті-майстрів через IG." },
  handmade: { dmToSaleRate: 0.10, tripwireToFlagship: 0.18, suggestedTripwire: 15, suggestedFlagship: 97, source: "Etsy Statistics 2026", insight: "Персоналізовані товари: 10% конверсія." },
  consulting: { dmToSaleRate: 0.12, tripwireToFlagship: 0.30, suggestedTripwire: 47, suggestedFlagship: 497, source: "Creatorflow 2026", insight: "Експерти: 12% конверсія, високий чек." },
  other: { dmToSaleRate: 0.10, tripwireToFlagship: 0.20, suggestedTripwire: 27, suggestedFlagship: 197, source: "ManyChat 2026", insight: "Стандартна Telegram воронка." },
};

const nicheLabels = {
  coaching: "Коучинг", fitness: "Фітнес", education: "Освіта", beauty: "Б'юті", handmade: "Хендмейд", consulting: "Консалтинг", other: "Інше",
};

const packageData = {
  1: { name: "Один продукт", investment: 495, support: 150, platforms: 45, description: "Один основний продукт з автоматизованою воронкою продажів", hasSecondProduct: false },
  2: { name: "Два продукти", investment: 990, support: 250, platforms: 45, description: "Недорогий вхідний + основний продукт — повна воронка", hasSecondProduct: true },
};

function AnimatedNumber({ value, prefix = "", suffix = "" }) {
  const [displayValue, setDisplayValue] = useState(value);
  const previousValue = useRef(value);

  useEffect(() => {
    const start = previousValue.current;
    const end = value;
    if (start === end) return;

    const duration = 400;
    const startTime = Date.now();

    const animate = () => {
      const elapsed = Date.now() - startTime;
      const progress = Math.min(elapsed / duration, 1);
      const easeOut = 1 - Math.pow(1 - progress, 3);
      const current = Math.round(start + (end - start) * easeOut);
      setDisplayValue(current);

      if (progress < 1) requestAnimationFrame(animate);
      else previousValue.current = value;
    };
    requestAnimationFrame(animate);
  }, [value]);

  const formatted = Math.abs(displayValue).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  return <span>{prefix}{formatted}{suffix}</span>;
}

export default function Calculator() {
  const [mode, setMode] = useState("manual");
  const [selectedPackage, setSelectedPackage] = useState(1);
  const [products, setProducts] = useState([{ id: 1, price: 197, sales: 5, type: "flagship" }]);
  const [includeSupport, setIncludeSupport] = useState(true);

  const [leads, setLeads] = useState(30);
  const [niche, setNiche] = useState("coaching");
  const [desiredIncome, setDesiredIncome] = useState(2000);

  const recommendation = useMemo(() => {
    const nicheInfo = nicheData[niche];
    const { dmToSaleRate, tripwireToFlagship, suggestedTripwire, suggestedFlagship } = nicheInfo;

    const warmLeads = Math.round(leads * 0.7);

    const tripwireConvRate = Math.min(0.5, dmToSaleRate * 2.5);
    const flagshipDirectRate = dmToSaleRate;

    const flagshipOnlySales = Math.max(1, Math.round(warmLeads * flagshipDirectRate));
    const tripwireSales = Math.max(1, Math.round(warmLeads * tripwireConvRate));
    const flagshipFromTripwire = Math.max(1, Math.round(tripwireSales * tripwireToFlagship));

    const pkg1NeededPrice = Math.round(desiredIncome / Math.max(1, flagshipOnlySales));
    const pkg1Reasonable = pkg1NeededPrice <= suggestedFlagship * 2;

    let recommendedPackage;
    let tripwirePrice = suggestedTripwire;
    let flagshipPrice = suggestedFlagship;
    let finalTripwireSales = 0;
    let finalFlagshipSales = flagshipOnlySales;
    let expectedIncome;

    if (pkg1Reasonable) {
      recommendedPackage = 1;
      flagshipPrice = Math.max(50, Math.min(2000, pkg1NeededPrice));
      finalFlagshipSales = flagshipOnlySales;
      finalTripwireSales = 0;
      expectedIncome = finalFlagshipSales * flagshipPrice;
    } else {
      recommendedPackage = 2;
      finalTripwireSales = tripwireSales;
      finalFlagshipSales = flagshipFromTripwire;

      const rawIncome = (finalTripwireSales * suggestedTripwire) + (finalFlagshipSales * suggestedFlagship);
      const scaleFactor = desiredIncome / Math.max(1, rawIncome);

      tripwirePrice = Math.max(10, Math.min(99, Math.round(suggestedTripwire * scaleFactor)));
      flagshipPrice = Math.max(100, Math.min(2000, Math.round(suggestedFlagship * scaleFactor)));

      expectedIncome = (finalTripwireSales * tripwirePrice) + (finalFlagshipSales * flagshipPrice);
    }

    const gap = Math.max(0, desiredIncome - expectedIncome);
    const isAchievable = gap <= 0;
    let neededLeads = leads;
    let growthTip = "";

    if (!isAchievable && expectedIncome > 0) {
      const incomePerLead = expectedIncome / leads;
      neededLeads = Math.ceil(desiredIncome / incomePerLead);

      const deficit = neededLeads - leads;
      if (deficit <= 20) {
        growthTip = `Потрібно лише +${deficit} заявок. Додайте лід-магніт або серію Stories з закликом — цього достатньо.`;
      } else if (deficit <= 50) {
        growthTip = `Потрібно +${deficit} заявок. Запустіть колаборації з 2-3 блогерами у вашій ніші або додайте Reels-воронку.`;
      } else {
        growthTip = `Потрібно +${deficit} заявок. Розгляньте таргетовану рекламу або партнерські запуски для швидкого масштабування.`;
      }
    }

    const tripwireRevenue = recommendedPackage === 2 ? finalTripwireSales * tripwirePrice : 0;
    const flagshipRevenue = finalFlagshipSales * flagshipPrice;

    return {
      package: recommendedPackage,
      tripwirePrice, tripwireSales: finalTripwireSales,
      flagshipPrice, flagshipSales: finalFlagshipSales,
      expectedIncome,
      tripwireRevenue,
      flagshipRevenue,
      gap,
      isAchievable,
      neededLeads,
      growthTip,
      insight: nicheInfo.insight,
    };
  }, [niche, leads, desiredIncome]);

  const applyRecommendations = () => {
    setSelectedPackage(recommendation.package);
    if (recommendation.package === 1) {
      setProducts([{ id: 1, price: recommendation.flagshipPrice, sales: recommendation.flagshipSales, type: "flagship" }]);
    } else {
      setProducts([
        { id: 1, price: recommendation.tripwirePrice, sales: recommendation.tripwireSales, type: "tripwire" },
        { id: 2, price: recommendation.flagshipPrice, sales: recommendation.flagshipSales, type: "flagship" }
      ]);
    }
    setMode("manual");
  };

  const handlePackageChange = (pkg) => {
    setSelectedPackage(pkg);
    if (pkg === 1) {
      const flagship = products.find(p => p.type === "flagship") || { id: 1, price: 197, sales: 5, type: "flagship" };
      setProducts([{ ...flagship, id: 1 }]);
    } else {
      if (products.length === 1) {
        const flagship = products[0];
        setProducts([
          { id: 1, price: 27, sales: 15, type: "tripwire" },
          { id: 2, price: flagship.price, sales: Math.max(1, Math.round(flagship.sales * 0.25)), type: "flagship" }
        ]);
      }
    }
  };

  const updateProduct = (id, field, value) => {
    setProducts(products.map(p => p.id === id ? { ...p, [field]: value } : p));
  };

  const isHelperMode = mode === "helper";

  const effectivePackage = isHelperMode ? recommendation.package : selectedPackage;
  const monthlyRevenue = isHelperMode ? recommendation.expectedIncome : products.reduce((sum, p) => sum + p.price * p.sales, 0);
  const pkg = packageData[effectivePackage];
  const monthlyCosts = (includeSupport ? pkg.support : 0) + pkg.platforms;
  const netProfit = monthlyRevenue - monthlyCosts;
  const investment = pkg.investment;
  const paybackDays = netProfit > 0 ? Math.round((investment / netProfit) * 30) : null;
  const isProfitable = netProfit > 0;

  return (
    <div className="max-w-5xl mx-auto p-8 animate-in-view">

      <div className="text-center mb-10 mt-4">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 mb-4">
          Калькулятор Окупності (ROI)
        </h1>
        <p className="text-slate-500 font-medium max-w-xl mx-auto">
          Спрогнозуйте свій дохід та дізнайтеся, як швидко окупляться інвестиції в автоворонку.
        </p>
      </div>

      {/* Mode Switcher */}
      <div className="flex justify-center mb-10">
        <div className="inline-flex items-center gap-1 p-1.5 bg-white rounded-xl border border-slate-200 shadow-sm">
          <button
            onClick={() => setMode("manual")}
            className={clsx(
              "px-5 py-2 text-sm font-semibold rounded-lg transition-all",
              mode === "manual" ? "bg-indigo-50 text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-800"
            )}
          >
            Ввести свої дані
          </button>
          <button
            onClick={() => setMode("helper")}
            className={clsx(
              "px-5 py-2 text-sm font-semibold rounded-lg transition-all flex items-center gap-2",
              mode === "helper" ? "bg-indigo-50 text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-800"
            )}
          >
            <iconify-icon icon="solar:lightbulb-bolt-linear" width="16"></iconify-icon>
            Допоможіть розрахувати
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">

        {/* Left Column: Inputs */}
        <div className="lg:col-span-7 space-y-5">

          {mode === "helper" ? (
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-md space-y-5">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <iconify-icon icon="solar:magic-stick-3-linear" className="text-indigo-500" width="20"></iconify-icon>
                Майстер розрахунку
              </h3>

              {/* Niche */}
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">Ваша ніша</label>
                <div className="grid grid-cols-4 gap-2">
                  {Object.entries(nicheLabels).map(([key, label]) => (
                    <button
                      key={key}
                      onClick={() => setNiche(key)}
                      className={clsx(
                        "py-2 px-2 rounded-lg border text-xs font-medium transition-all text-center",
                        niche === key
                          ? "border-indigo-500 bg-indigo-50 text-indigo-700 shadow-sm"
                          : "border-slate-200 bg-white text-slate-600 hover:border-slate-300"
                      )}
                    >
                      {label}
                    </button>
                  ))}
                </div>
              </div>

              {/* Sliders */}
              <div className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-100">
                <div>
                  <div className="flex justify-between mb-1.5">
                    <label className="text-sm font-medium text-slate-700">Заявок / звернень на місяць</label>
                    <span className="text-sm font-bold text-indigo-600">{leads}</span>
                  </div>
                  <CustomSlider value={leads} onChange={setLeads} min={5} max={500} step={5} />
                  <p className="text-xs text-slate-400 mt-1">DM, заявки з форм, коментарі «хочу» тощо</p>
                </div>
                <div>
                  <div className="flex justify-between mb-1.5">
                    <label className="text-sm font-medium text-slate-700">Бажаний дохід ($/міс)</label>
                    <span className="text-sm font-bold text-emerald-600">${desiredIncome.toLocaleString()}</span>
                  </div>
                  <CustomSlider value={desiredIncome} onChange={setDesiredIncome} min={500} max={20000} step={500} />
                </div>
              </div>

              {/* Live Breakdown */}
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2.5">
                <div className="flex justify-between items-center text-sm pb-2 border-b border-slate-200">
                  <span className="text-slate-500 font-medium">Пакет</span>
                  <span className="font-bold text-slate-800">{packageData[recommendation.package].name}</span>
                </div>

                {recommendation.package === 2 && (
                  <>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500">
                        <iconify-icon icon="solar:star-fall-linear" className="text-amber-500 mr-1" width="13"></iconify-icon>
                        Вхідний: ${recommendation.tripwirePrice} × {recommendation.tripwireSales}
                      </span>
                      <span className="font-semibold text-slate-700">${recommendation.tripwireRevenue.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500">
                        <iconify-icon icon="solar:crown-star-linear" className="text-indigo-500 mr-1" width="13"></iconify-icon>
                        Основний: ${recommendation.flagshipPrice} × {recommendation.flagshipSales}
                      </span>
                      <span className="font-semibold text-slate-700">${recommendation.flagshipRevenue.toLocaleString()}</span>
                    </div>
                  </>
                )}

                {recommendation.package === 1 && (
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-500">
                      <iconify-icon icon="solar:crown-star-linear" className="text-indigo-500 mr-1" width="13"></iconify-icon>
                      Основний: ${recommendation.flagshipPrice} × {recommendation.flagshipSales}
                    </span>
                    <span className="font-semibold text-slate-700">${recommendation.flagshipRevenue.toLocaleString()}</span>
                  </div>
                )}

                <div className="flex justify-between items-center text-sm pt-2 border-t border-slate-200">
                  <span className="text-slate-500 font-medium">Дохід</span>
                  <span className="font-bold text-emerald-600">${recommendation.expectedIncome.toLocaleString()}/міс</span>
                </div>
              </div>

              {!recommendation.isAchievable && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 space-y-1.5">
                  <div className="flex items-center gap-2">
                    <iconify-icon icon="solar:info-circle-linear" className="text-amber-600" width="16"></iconify-icon>
                    <span className="text-xs font-semibold text-amber-900">Потрібно більше заявок</span>
                  </div>
                  <p className="text-xs text-amber-800 leading-relaxed">
                    Досяжно <strong>${recommendation.expectedIncome.toLocaleString()}</strong>/міс. Для ${desiredIncome.toLocaleString()} потрібно ~<strong>{recommendation.neededLeads}</strong> заявок.
                  </p>
                  {recommendation.growthTip && (
                    <p className="text-xs text-amber-700 pt-1 border-t border-amber-200">{recommendation.growthTip}</p>
                  )}
                </div>
              )}

              {recommendation.isAchievable && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-center gap-2">
                  <iconify-icon icon="solar:verified-check-linear" className="text-emerald-600" width="16"></iconify-icon>
                  <span className="text-xs font-semibold text-emerald-800">Ціль ${desiredIncome.toLocaleString()}/міс досяжна!</span>
                </div>
              )}

              <button
                onClick={applyRecommendations}
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold shadow-sm transition-all active:scale-[0.98] flex justify-center items-center gap-2"
              >
                <iconify-icon icon="solar:check-read-linear" width="16"></iconify-icon>
                Застосувати та налаштувати вручну
              </button>
            </div>
          ) : (
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-5">

              {/* Package Selector — pill toggle */}
              <div className="space-y-3">
                <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Вибір пакету</h3>
                <div className="inline-flex items-center p-1 bg-slate-100 rounded-lg border border-slate-200">
                  {[1, 2].map(num => (
                    <button
                      key={num}
                      onClick={() => handlePackageChange(num)}
                      className={clsx(
                        "px-4 py-2 text-sm font-semibold rounded-md transition-all",
                        selectedPackage === num
                          ? "bg-white text-indigo-700 shadow-sm border border-slate-200"
                          : "text-slate-500 hover:text-slate-700"
                      )}
                    >
                      {packageData[num].name}
                    </button>
                  ))}
                </div>
                <p className="text-xs text-slate-400">
                  {pkg.description} — <span className="font-semibold text-indigo-600">${pkg.investment}</span> разова інвестиція
                </p>
              </div>

              {/* Product Inputs — compact horizontal rows */}
              <div className="space-y-4">
                <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Продажі та Ціни</h3>

                {products.map((product) => (
                  <div key={product.id} className="bg-slate-50 rounded-xl border border-slate-100 p-4 space-y-3">
                    <div>
                      <div className="flex items-center gap-2 text-sm font-bold text-slate-800">
                        {product.type === "tripwire"
                          ? <iconify-icon icon="solar:star-fall-linear" className="text-amber-500" width="16"></iconify-icon>
                          : <iconify-icon icon="solar:crown-star-linear" className="text-indigo-500" width="16"></iconify-icon>
                        }
                        {product.type === "tripwire" ? "Вхідний продукт" : "Основний продукт"}
                      </div>
                      <p className="text-xs text-slate-400 mt-0.5 ml-6">
                        {product.type === "tripwire"
                          ? "Недорога пропозиція для першого знайомства (чек-лист, міні-курс, гайд)"
                          : "Ваш головний продукт з максимальною цінністю (курс, менторство, послуга)"
                        }
                      </p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="flex justify-between mb-1">
                          <label className="text-xs font-medium text-slate-500">Ціна</label>
                          <span className="text-xs font-bold text-slate-700">${product.price}</span>
                        </div>
                        <CustomSlider
                          value={product.price}
                          onChange={(v) => updateProduct(product.id, "price", v)}
                          min={product.type === "tripwire" ? 5 : 50}
                          max={product.type === "tripwire" ? 100 : 2000}
                          step={product.type === "tripwire" ? 1 : 10}
                        />
                      </div>
                      <div>
                        <div className="flex justify-between mb-1">
                          <label className="text-xs font-medium text-slate-500">Продажів/міс</label>
                          <span className="text-xs font-bold text-slate-700">{product.sales} шт.</span>
                        </div>
                        <CustomSlider
                          value={product.sales}
                          onChange={(v) => updateProduct(product.id, "sales", v)}
                          min={1} max={100} step={1}
                        />
                      </div>
                    </div>
                  </div>
                ))}
              </div>

              {/* Monthly Costs — compact row */}
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                <div className="flex items-center gap-3">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" className="sr-only peer" checked={includeSupport} onChange={(e) => setIncludeSupport(e.target.checked)} />
                    <div className="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-indigo-600"></div>
                  </label>
                  <div>
                    <span className="text-sm font-medium text-slate-700">Тех. підтримка</span>
                    <span className="text-xs text-slate-400 ml-1">(${pkg.support}/міс)</span>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-xs text-slate-400">+ платформи ${pkg.platforms}/міс</span>
                </div>
              </div>

            </div>
          )}
        </div>

        {/* Right Column: Results sticky */}
        <div className="lg:col-span-5">
          <div className="sticky top-8 space-y-5">

            <div className="bg-slate-900 text-white rounded-2xl p-7 shadow-xl">
              <h2 className="text-lg font-bold mb-5 border-b border-slate-700 pb-3">Фінансовий результат</h2>

              <div className="space-y-3 mb-6">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-400">Дохід на місяць</span>
                  <span className="font-mono text-lg font-semibold"><AnimatedNumber value={monthlyRevenue} prefix="$" /></span>
                </div>
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-400">Витрати на місяць</span>
                    <span className="font-mono text-lg font-semibold text-rose-400">-<AnimatedNumber value={monthlyCosts} prefix="$" /></span>
                  </div>
                  <div className="ml-3 space-y-1 border-l-2 border-slate-700 pl-3">
                    {includeSupport && (
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-500">Тех. підтримка</span>
                        <span className="text-slate-500 font-mono">${pkg.support}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-500">Платформи (бот, хостинг)</span>
                      <span className="text-slate-500 font-mono">${pkg.platforms}</span>
                    </div>
                  </div>
                </div>
                <div className="h-px bg-slate-700 w-full my-1"></div>
                <div className="flex justify-between items-center">
                  <span className="font-bold text-slate-200">Чистий прибуток</span>
                  <span className={clsx("font-mono text-3xl font-bold tracking-tight", isProfitable ? "text-emerald-400" : "text-rose-400")}>
                    <AnimatedNumber value={netProfit} prefix="$" />
                  </span>
                </div>
              </div>

              <div className="bg-slate-800 rounded-xl p-4 border border-slate-700/50">
                <div className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-2">Окупність інвестицій (${investment})</div>

                {isProfitable ? (
                  <div className="flex items-end gap-3">
                    <span className="text-4xl font-bold text-white tracking-tight"><AnimatedNumber value={paybackDays} /></span>
                    <span className="text-slate-400 font-medium mb-1">днів</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 text-rose-400 text-sm font-medium">
                    <iconify-icon icon="solar:danger-triangle-linear" width="18"></iconify-icon>
                    Поточна модель збиткова
                  </div>
                )}

                {isProfitable && (
                  <div className="mt-3 pt-3 border-t border-slate-700/50 text-sm">
                    <span className="text-slate-400">Річний прогноз: </span>
                    <span className="font-bold text-emerald-400 ml-1">+${((netProfit * 12) - investment).toLocaleString()}</span>
                  </div>
                )}
              </div>
            </div>

            {!isProfitable && (
              <div className="bg-rose-50 border border-rose-200 rounded-2xl p-5 shadow-sm">
                <div className="flex items-start gap-3">
                  <iconify-icon icon="solar:info-circle-linear" className="text-rose-500 text-lg mt-0.5 flex-shrink-0"></iconify-icon>
                  <div>
                    <h4 className="font-bold text-rose-900 mb-1 text-sm">Як вийти в плюс?</h4>
                    <ul className="text-xs text-rose-800 space-y-1.5 list-disc pl-3 font-medium">
                      <li>Збільшіть ціну хоча б одного продукту</li>
                      <li>Залучіть більше трафіку для збільшення продажів</li>
                      {includeSupport && <li>Вимкніть тех. підтримку (−${pkg.support}/міс)</li>}
                    </ul>
                  </div>
                </div>
              </div>
            )}

            {isProfitable && (
              <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5 shadow-sm flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-emerald-900 mb-0.5 text-sm">
                    {isHelperMode ? "Прогноз позитивний" : "Відмінний результат"}
                  </h4>
                  <p className="text-xs text-emerald-700">
                    {isHelperMode
                      ? "Модель прибуткова за рекомендованими параметрами."
                      : "Модель виглядає життєздатною та прибутковою."
                    }
                  </p>
                </div>
                <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600">
                  <iconify-icon icon="solar:chart-2-linear" width="20"></iconify-icon>
                </div>
              </div>
            )}

          </div>
        </div>

      </div>
    </div>
  );
}
